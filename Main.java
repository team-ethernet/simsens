...
public class Main {
	...
	public static void multiMsgLoop(String brokerIp, String topic, String nodeId, int numOfMsg, int msgDelay, int db) throws IOException, InterruptedException {
		//Sending messages		        
		db = dB();
		sendmsg(brokerIp, topic, nodeId, db+"", numOfMsg+"", msgDelay+"");
	}
	
	//Message sending function
	public static void sendmsg(String brokerIp, String topic, String nodeId, String db, String numOfMsg, String msgDelay) throws IOException {
		ProcessBuilder prcBldr = new ProcessBuilder();
		prcBldr.command("./mosquittotester.bat", brokerIp, topic, nodeId, db, numOfMsg, msgDelay);
		prcBldr.start();
		//System.out.println("Msg sent: " + "{\"node_id\":\"" + nodeId + "\",\"db\":" + db + "}");
		System.out.println("Sending rest of messages quietly...");
	}
}
